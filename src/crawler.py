...
def crawl(year, month, driver_path, output_path):
    ...
    sleep(5)
    select_month_and_year(year, month, driver)
...
def select_month_and_year(year, month, driver):
    ...
    # Esse Select serve para pegar as tags select, onde posso passar o valor diretamente,
...
def download_remuneracao(driver, output_path, month, year):
    ...
    filename = "membros-ativos-contracheque-" + month + "-" + year + ".csv"
...
def download_indenizacao(driver, output_path, month, year):
    data = find_trs(driver)
    # Dá um nome para o arquivo
    filename = "membros-ativos-verbas-indenizatorias-" + month + "-" + year + ".csv"
    path = output_path + "/" + filename
